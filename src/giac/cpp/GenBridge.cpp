...
GenBridge::GenBridge(string expression, ContextBridge& context) {
	...
	std::cout << "[DEBUG] " << __FUNCTION__ << "for expression " << expression << " created gen " << g  << " with type " << g->type << " and a vector " << ((g->type == 7) ? g->_VECTptr : (giac::vecteur*)0) <<"\n";
}
...
GenBridge::~GenBridge() {
	std::cout << "[DEBUG] " << __FUNCTION__ << " called for giac::gen: " << g << "\n";
	delete g;
}
...
int GenBridge::lenght() {
	std::cout << "[DEBUG] " << __FUNCTION__ << " called for gen " << g  << "\n";
	if (g->type == 7) { // type is vector
		giac::vecteur* v = g->_VECTptr;
		std::cout << "[DEBUG] " << __FUNCTION__ << " called  on a vector: " << v << "\n";
		return v->size();
	} else {
		return 1;
	}
